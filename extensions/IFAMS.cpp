...
NFD_LOG_INIT(IFAMS);
NFD_REGISTER_STRATEGY(IFAMS);

const time::milliseconds IFAMS::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds IFAMS::RETX_SUPPRESSION_MAX(250);

IFAMS::IFAMS(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
...
const Name&
IFAMS::getStrategyName()
{
  static Name strategyName("/localhost/nfd/strategy/IFAMS/%FD%05");
  return strategyName;
}

void
IFAMS::afterReceiveInterest(const FaceEndpoint& ingress, const Interest& interest,
...
void
IFAMS::afterReceiveNack(const FaceEndpoint& ingress, const lp::Nack& nack,
                                     const shared_ptr<pit::Entry>& pitEntry)
{
