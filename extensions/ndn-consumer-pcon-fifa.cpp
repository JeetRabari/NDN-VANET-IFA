...
#include "ndn-consumer-pcon-fifa.hpp"

NS_LOG_COMPONENT_DEFINE("ndn.ConsumerPconFifa");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(ConsumerPconFifa);

constexpr double ConsumerPconFifa::CUBIC_C;
constexpr uint32_t ConsumerPconFifa::BIC_MAX_INCREMENT;
constexpr uint32_t ConsumerPconFifa::BIC_LOW_WINDOW;

TypeId
ConsumerPconFifa::GetTypeId()
{
  static TypeId tid =
    TypeId("ns3::ndn::ConsumerPconFifa")
      .SetGroupName("Ndn")
      .SetParent<ConsumerWindowFifa>()
      .AddConstructor<ConsumerPconFifa>()

      .AddAttribute("CcAlgorithm",
                    "Specify which window adaptation algorithm to use (AIMD, BIC, or CUBIC)",
                    EnumValue(CcAlgorithm::AIMD),
                    MakeEnumAccessor(&ConsumerPconFifa::m_ccAlgorithm),
      ...
      .AddAttribute("Beta",
                    ...
                    DoubleValue(0.5),
                    MakeDoubleAccessor(&ConsumerPconFifa::m_beta),
                    MakeDoubleChecker<double>())

      .AddAttribute("CubicBeta",
                    "TCP CUBIC Multiplicative Decrease factor",
                    DoubleValue(0.8),
                    MakeDoubleAccessor(&ConsumerPconFifa::m_cubicBeta),
                    MakeDoubleChecker<double>())
      ...
      .AddAttribute("AddRttSuppress",
                    ...
                    MakeDoubleAccessor(&ConsumerPconFifa::m_addRttSuppress),
                    MakeDoubleChecker<double>())

      .AddAttribute("ReactToCongestionMarks",
                    "If true, process received congestion marks",
                    BooleanValue(true),
                    MakeBooleanAccessor(&ConsumerPconFifa::m_reactToCongestionMarks),
                    MakeBooleanChecker())

      .AddAttribute("UseCwa",
                    "If true, use Conservative Window Adaptation",
                    BooleanValue(true),
                    MakeBooleanAccessor(&ConsumerPconFifa::m_useCwa),
                    MakeBooleanChecker())

      .AddAttribute("UseCubicFastConvergence",
                    "If true, use TCP CUBIC Fast Convergence",
                    BooleanValue(false),
                    MakeBooleanAccessor(&ConsumerPconFifa::m_useCubicFastConv),
                    MakeBooleanChecker());

  return tid;
}

ConsumerPconFifa::ConsumerPconFifa()
...
void
ConsumerPconFifa::OnData(shared_ptr<const Data> data)
{
  ConsumerFifa::OnData(data);
  ...
  // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  //
  uint64_t sequenceNum = data->getName().get(-3).toSequenceNumber();
...
void
ConsumerPconFifa::OnTimeout(uint32_t sequenceNum)
{
  WindowDecrease();
  ...
  ConsumerFifa::OnTimeout(sequenceNum);
}

void
ConsumerPconFifa::WindowIncrease()
{
...
void
ConsumerPconFifa::WindowDecrease()
{
...
void
ConsumerPconFifa::CubicIncrease()
{
...
void
ConsumerPconFifa::CubicDecrease()
{
